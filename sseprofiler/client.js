...
function connect(isReconnect) {
  ...
  s.on('close', () => {
    ...
    S.state = 'closed';
    logErr('error', `${'ECONNRESET'} · closed by peer`, C.red);
    reconnectTimer = setTimeout(() => connect(true), S.retryMs);
  });
}
...
function handleBlock(block) {
  ...
  for (let line of block.split('\n')) {
    ...
    else if (line.startsWith('retry: ')) S.retryMs = +line.slice(7).trim() || S.retryMs;
  ...
  try {
    ...
    if (o.ts) { lat = Math.round(Date.now() + S.offset.offset - o.ts); record(lat); }
  } catch {}
  if (!channel) channel = '-';
...
function collectLoad() {
  ...
  let conns = 0, opening = 0, msgs = 0, max = 0, errTotal = 0;
  const errs = {};
  for (const s of live) {
    if (!s) continue;
  ...
  S.load = { conns, opening, msgs, errs, errTotal, max, hist: h, total,
...
function render() {
  ...
  const conn = ['',
    `  ${C.gray}state${C.rst}${padS(`${C.green}${S.state}${C.rst}${C.gray} · ${S.connectedAt ? Math.round((Date.now() - S.connectedAt) / 1000) + 's' : '-'}${C.rst}`, w1 - 10)}`,
    ...
    `  ${C.gray}events received${C.rst}${padS(`${C.cyan}${num(S.total)}${C.rst}`, w1 - 20)}`,
  ...
  if (S.filter) {
    ...
    feed = feed.filter((e) => (e.event + e.channel + e.data).toLowerCase().includes(f));
  }
  for (const e of feed.slice(-Math.max(3, bodyRows - 3))) {
    const d = e.t;
    ...
    if (has('lat')) line += (e.lat == null ? C.gray + padS('--', 6) : heat(e.lat, 250, 1000) + padS(e.lat + 'ms', 6)) + ' ' + C.rst;
...
function bindKeys() {
  ...
  process.stdin.on('data', (k) => {
    ...
    else if (k === 'r') { S.state = 'closing'; sock?.destroy(); connect(true); }
...
function quit() {
  ...
  console.log(`reconnects                    ${S.reconnects} (${S.resumed} resumed via Last-Event-ID)`);
...
function runLoadWorker(cfg) {
  ...
  function onData(buf) {
    ...
    for (;;) {
      ...
      rec(now - Number(str.slice(start, e)));
      curs = e;
    }
  ...
  const open = () => {
    const port = cfg.ports[opened % cfg.ports.length];
    const localAddress = cfg.local[opened % cfg.local.length] || undefined;
    opened++; opening++;
    ...
    s.on('close', () => { if (s.connecting) opening--; else connected--; });
  };
  ...
  setInterval(() => {
    parentPort.postMessage({ connected, opening, msgs, max, errs: { ...errs }, hist: Int32Array.from(hist) });
    hist.fill(0); msgs = 0; max = 0;
